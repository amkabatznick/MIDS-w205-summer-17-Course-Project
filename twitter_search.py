...
cur.execute("SELECT t3.facet_details FROM article_details AS t1 
INNER JOIN article_facet_details AS t2 ON t1.article_id = t2.article_id 
INNER JOIN facet_details AS t3 ON t2.facet_detail_id = t3.facet_detail_id 
WHERE t1.update_date > %s;",(two_days_ago,))
